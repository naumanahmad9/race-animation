...
function App() {
  ...
  useEffect(() => {
    ...
    const firstBackgroundMovement = firstBackground.current.animate(
      sceneryFrames,
      sceneryTimingBackground
    );
    firstBackgroundMovement.currentTime =
      firstBackgroundMovement.effect.getTiming().duration / 2;

    const secondBackgroundMovement = secondBackground.current.animate(
      sceneryFrames,
      sceneryTimingBackground
    );

    const firstForegroundMovement = firstForeground.current.animate(
      sceneryFrames,
      sceneryTimingForeground
    );
    firstForegroundMovement.currentTime =
      firstForegroundMovement.effect.getTiming().duration / 2;

    const secondForegroundMovement = secondForeground.current.animate(
      sceneryFrames,
      sceneryTimingForeground
    );

    const aliceAnimationMovement = aliceAnimation.current.animate(
      spriteFrames,
      {
        easing: "steps(7,end)",
        direction: "reverse",
    ...
    var scenes = [
      firstForegroundMovement,
      secondForegroundMovement,
      firstBackgroundMovement,
      secondBackgroundMovement,
    ];

    const adjustPlaybackRate = () => {
      if (aliceAnimationMovement.playbackRate < 0.8) {
        scenes.forEach((animation) => {
          animation.playbackRate =
            (aliceAnimationMovement.playbackRate / 2) * -1;
        });
    ...
    setInterval(() => {
      if (aliceAnimationMovement.playbackRate > 0.4) {
